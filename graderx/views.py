    ...
    if method == "file":
        ...
        if allowed_file(submissions_file.filename):
            ...
            try:
                ...
                return jsonify({
                    "message": UPLOAD_STATUS.SUCCESS.value, 'key': submission_key
                }), 200
            except:
                return jsonify({
...
def get_submission_file_content():
    try:
        ...
        return jsonify({'message': "course, lab, submission_id, file_name and submission key query parameters must be included"}), 400
    try:
