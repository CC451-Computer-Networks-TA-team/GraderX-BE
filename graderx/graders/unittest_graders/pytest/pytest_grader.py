...
def add_submissions(course, lab, submissions_file, key=None):
    """
...
def save_single_submission(course, lab, file_in_memory, filename, key=None):
    lab_path = get_lab_path(course, lab)
    lab_path.joinpath(f'submissions/2020/{filename}').write_bytes(
        file_in_memory.getbuffer())


def clear_submissions(course, lab, key=None):
    lab_path = get_lab_path(course, lab)
...
def get_submission_files(course, lab, submission_id, key=None):
    lab_path = get_lab_path(course, lab)
    return list(lab_path.glob(f"submissions/2020/{submission_id}.py"))

def update_submission_files(course, lab, submission_id, submission_files, key=None):
...
def get_submission_file_content(course, lab, submission_id, file_name, key=None):
    submission_path = get_lab_path(course, lab).joinpath(f'submissions/2020/')
    submission_file = open(submission_path.joinpath(file_name))
    return submission_file.read()

def get_not_fullmark_submissions(course, lab, key=None):
    lab_path = get_lab_path(course, lab)
