...
def compute_total_result(result_dict):
...
def get_path(student_id):
    the_path = LAB_ABS_PATH.joinpath('submissions').joinpath(student_id)
    return the_path


def get_lab_name():
    LAB_NAME = str(LAB_ABS_PATH).replace(str(COURSE_ABS_PATH),"").strip("/")
    return LAB_NAME

   
def write_internal_report(content, path):
    file_name = path.joinpath(f'{get_lab_name()}_results.txt')
...
def write_summary_report(content, path):
    file_name = path.joinpath(f'{get_lab_name()}_result_summary.txt')
    ...
    with open(file_name, 'w') as out_file:
        out_file.write(f'* {get_lab_name()} Result Summary\n')
        out_file.write(f'--'*10 + '\n')
