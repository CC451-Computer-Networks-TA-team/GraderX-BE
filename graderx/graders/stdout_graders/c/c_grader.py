...
def get_lab_path(course, lab, key):
    return Path(__file__).joinpath(
        f'../../../courses/{course}/labs/{lab}/{key}').resolve()


def run_grader(course, lab, key):
    """
    ...
    """
    LAB_ABS_PATH = get_lab_path(course, lab, key)
...
def add_submissions(course, lab, submissions_file, key):
    """
    ...
    """
    lab_path = get_lab_path(course, lab, key)
...
def save_single_submission(course, lab, file_in_memory, filename, key):
    lab_path = get_lab_path(course, lab, key)
    lab_path.joinpath(f'submissions/{filename}').write_bytes(
        file_in_memory.getbuffer())


def clear_submissions(course, lab, key):
    lab_path = get_lab_path(course, lab, key)
...
def get_diff_results_file(course_name, lab, key):
    path = Path("graderx").joinpath("graders").joinpath("courses")
    path.joinpath(course_name).joinpath("labs").joinpath(lab).joinpath(key)
...
def update_submission_files(course, lab, submission_id, submission_files, key):
    submission_path = get_lab_path(course, lab, key).joinpath(f'submissions/{submission_id}')
...
def get_not_fullmark_submissions(course, lab, key):
    lab_path = get_lab_path(course, lab, key)
