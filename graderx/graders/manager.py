...
def run_grader_commands(course_code, lab_id):
    ...
    cmd = shlex.split(
        f"pytest -vv --tb=short --show-capture=no {curr_dir}/courses/{course_code}/app/{lab_id}/test_run_grader.py")
...
def run_grader(course_code: str, lab_id: str, submissions_file: FileStorage) -> dict:
    ...
    extract_submissions(Path(
        f"{curr_dir}/courses/{course_code}/app/{lab_id}/submissions/2020"), submissions_file)
